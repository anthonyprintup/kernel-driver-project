...
asn1der::Certificate::EpochTimeType utcTimeSinceEpoch() noexcept {
	...
	nt::KeQuerySystemTimePrecise(&currentTime);
	ULONG gmtUnixTime {};
	nt::RtlTimeToSecondsSince1970(&currentTime, &gmtUnixTime);
	VM_SIZE_SPEED_END
...
bool Certificate::valid(const Certificate *issuer, const std::string_view hostname) const noexcept {
	...
	if (timeSinceEpoch < this->parsedCertificate.notBefore || timeSinceEpoch > this->parsedCertificate.notAfter) {
		PRINT_IF_DEBUG(xorstr_("[!] Certificate vailidity is invalid for %s"), this->parsedCertificate.subjectName.commonName)
		return false;
	}
	...
	if (!hostname.empty()) {
		...
		if (commonName.empty()) {
			PRINT_IF_DEBUG(xorstr_("[!] Missing subject name common name"))
			return false;
		}
		...
		if (!matchedName) {
			PRINT_IF_DEBUG(xorstr_("[!] Failed to find a name to match for %s"), this->parsedCertificate.subjectName.commonName)
			return false;
		}
	}

	if (this->parsedCertificate.publicKey.encryptionIdentifier != asn1der::identifiers::rsaEncryption) {
		PRINT_IF_DEBUG(xorstr_("[!] Unsupported encryption algorithm for %s"), this->parsedCertificate.subjectName.commonName)
		return false;
	}
	...
	{ // Import modulus & exponent
		...
		if (issuer) {
			...
			// ReSharper disable once CppTooWideScope
			const Hash issuerCommonName = this->parsedCertificate.issuerName.commonName;
			if (issuerCommonName == Fnv1A("DigiCert Global Root G2")) {
				modulus  = certificates::DigiCertGlobalRootG2::modulus;
				exponent = certificates::DigiCertGlobalRootG2::exponent;
			} else if (issuerCommonName == Fnv1A("Baltimore CyberTrust Root")) {
				modulus  = certificates::BaltimoreCyberTrustRoot::modulus;
				exponent = certificates::BaltimoreCyberTrustRoot::exponent;
			} else {
				PRINT_IF_DEBUG(xorstr_("[!] Failed to identify the root issuer for %s"), this->parsedCertificate.subjectName.commonName)
				return false;
			}
		}
	...
	if (const auto error = libtomcrypt::ltc_mp.rsa_me(
		...
		PRINT_IF_DEBUG(xorstr_("[!] Failed to decode the certificate fingerprint using the issuer's public key for: %s"), this->parsedCertificate.subjectName.commonName)
		return false;
	}
	...
	if (int valid {}, error = libtomcrypt::pkcs_1_v1_5_decode(
		...
		PRINT_IF_DEBUG(xorstr_("[!] Failed to decode the hash sequence."))
		return false;
	}
	...
	if (const auto successfullyParsed = asn1der::parser::tag(&hashStructure, hashSequenceReader);
		!successfullyParsed) {
		PRINT_IF_DEBUG(xorstr_("[!] Failed to parse the hash sequence."))
		return false;
	}
