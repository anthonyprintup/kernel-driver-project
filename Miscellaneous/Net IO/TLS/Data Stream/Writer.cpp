...
tls::VectorType::pointer Writer::expand(const std::size_t size) {
	if (!size) return {};

	const auto currentOffset = this->size();
	this->_data.resize(currentOffset + size);
	return this->data() + currentOffset;
}
